...
void crc32_init(crc32_ctx_t *ctx)
{
    ctx->crc = 0 ^ 0xFFFFFFFFu; // zlib initial value 0 is XORed in
}
...
{
    ctx->crc = crc32_tab[(ctx->crc ^ b) & 0xFFu] ^ (ctx->crc >> 8);
}

uint32_t crc32_get(const crc32_ctx_t *ctx)
{
    return ctx->crc ^ 0xFFFFFFFFu; // final XOR
}

uint32_t crc32_calc(const void *data, size_t len)
{
    crc32_ctx_t c; crc32_init(&c); crc32_update(&c, data, len); return crc32_get(&c);
}
